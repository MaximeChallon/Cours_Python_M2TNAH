...
def lieu_update(place_id):
    ...
    # prendre en compte la modification dans la bdd
    if request.method == 'POST':
        # dans form.get, le deuxième paramètre est la valeur par défaut qui est l'ancienne valeur de la bdd
        lieu.place_nom=request.form.get("place_nom", lieu.place_nom)
        # ...
        db.session.add(lieu)
        db.session.commit()
    # gérer l'affichage du formulaire
    return render_template("pages/place_edit.html", nom="Gazetteer", lieu=lieu)